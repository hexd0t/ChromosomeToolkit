...
fn main() {
    println!("Chromosome Toolkit - R1 - LRENT to JSON");
    ...
    while let Some(arg) = queue.pop_front() {
        ...
        if path.is_dir() {
            if let Ok(dir) = path.read_dir() {
                for file in dir.flatten() {
                    ...
                    if meta.is_dir() || path.ends_with(".lrent") || path.ends_with(".LRENT") {
        ...
        let lrent = match LrentFile::load(arch) {
            Ok(a) => a,
            Err(e) => {
                println!("loading lrent failed: {e}, skipping");
                continue;
            }
        };
        ...
        let out_arg = arg.replace(".lrent", ".lrent.json");
        if out_arg == arg {
            panic!("In == out path");
        }
        ...
        lrent.serialize(&mut ser).unwrap();
